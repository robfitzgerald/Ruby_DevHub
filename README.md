...
Before using the sample application please locate the src\request\Config.rb 
You'll need to enter the license Id you obtained in the portal into the "license" field. To learn more about how the code is setting the license, please reference the file src\request\Request.rb to see how it's set in the HTTPS header.
...
* The src\Test.rb provides a way to begin trying out the different API transaction types. 
...
In the file src\Test.rb you can simply uncomment the transactions you'd like to try out. For example, the following will send an example of an Authorization transactions.
...
Although the src\Test.rb will handle setting everything for you, we wanted to call out how it works. Please reference the src\request\Request.rb The "send()" function handles converting the object to a Json string as well as setting the header information for the request.
...
The sample demonstrates obtaining the string response using the following in the src\request\Request.rb The "getResponse()" function. 
...
An example of locating specific meta data in the response can be found by referencing the src\request\Utilities.rb The "getTransactionID()" function. In this case we are extracting the TransactionID. 
...
```
> src
	> Test.rb : Used as a starting point to try out the API using the Sample Code.
	> request (folder)
		> Config.rb: Used to set the base endpoint URL, license, and optional proxy settings should your company have a proxy.
		> Request.rb: Contains examples of sending a tranaction, converting an object to JSON as well as serializing the response to JSON.
		> SampleRequests.rb: The following data generator examples provide test values that can be set per each transaction type. 
		> Utilities.rb: Useful example of code like extracting values from a string response. 
